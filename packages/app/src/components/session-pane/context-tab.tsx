...
export interface ContextTabProps {
  sessionId: string
  sessionInfo?: { title?: string; time?: { created?: number } }
}
...
export function ContextTab(props: ContextTabProps) {
  ...
  const systemPrompt = createMemo(() => {
    const msg = userMessages().findLast((m) => !!m.system)
  ...
  const stats = createMemo(() => {
    ...
    return [
      ...
      { label: "Session Created", value: time(props.sessionInfo?.time?.created) },
      { label: "Last Activity", value: time(c?.message.time.created) },
    ] satisfies { label: string; value: JSX.Element }[]
  })

  function RawMessageContent(contentProps: { message: Message }) {
    const file = createMemo(() => {
      const parts = (sync.data.part[contentProps.message.id] ?? []) as Part[]
      const contents = JSON.stringify({ message: contentProps.message, parts }, null, 2)
      return {
        name: `${contentProps.message.role}-${contentProps.message.id}.json`,
        contents,
  ...
  function RawMessage(rawProps: { message: Message }) {
    return (
      <Accordion.Item value={rawProps.message.id}>
        <StickyAccordionHeader>
          <Accordion.Trigger>
            <div class="flex items-center justify-between gap-2 w-full">
              <div class="min-w-0 truncate">
                {rawProps.message.role} <span class="text-text-base">• {rawProps.message.id}</span>
              </div>
              <div class="flex items-center gap-3">
                <div class="shrink-0 text-12-regular text-text-weak">{time(rawProps.message.time.created)}</div>
        ...
        <Accordion.Content class="bg-background-base">
          <div class="p-3">
            <RawMessageContent message={rawProps.message} />
          </div>
        </Accordion.Content>
      </Accordion.Item>
    )
